  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if(searchCache[searchQuery]){
        ...
        // In getSearchSuggestions we will dispatch the cahed results.
      }
    }, 200);
    ...
    <div className="flex justify-between m-2">
      ...
      <div className="w-1/2 relative self-center">
        <div className="flex ">
          ...
          />
          <button className="rounded-r-full border border-gray-300 border-l-0 p-1 bg-gray-100 h-9">
            <img className="h-6" src={searchIcon} alt="search icon" />
          </button>
        </div>
        {showSuggestions && (
          <div className="absolute bg-white py-2 w-2/3 rounded-lg cursor-default">
            <ul>
